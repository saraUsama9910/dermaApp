...
class ProfileRepoImplementation implements ProfileRepo {
  ...
  ProfileRepoImplementation({required this.apiConsumer});
  @override
  ...
  Future<Either<String, String>> changePassword(
    ...
    try {
      final response = await apiConsumer.post(
          EndPoints.changePasswordEndPoint(userId: userId),
          data: {
  ...
  @override
  Future<Either<String, UserAllDataModel>> getUserData(
      {required num userId}) async {
    try {
      final response =
          await apiConsumer.get(EndPoints.getUserDataEndPoint(userId: userId));
      final UserAllDataModel userAllDataModel =
          UserAllDataModel.fromJson(response);
  ...
  Future<Either<String, String>> deleteUser(
      {required num userId, required String token}) async {
    try {
      final response = await apiConsumer
          .delete(EndPoints.deleteUserEndPoint(userId: userId, token: token));
      return Right(response[ApiKeys.message]);
    } on ServerException catch (e) {
      return Left(e.errorModel.errorMessage);
    }
