...
class CommunityRepoImplementation implements CommunityRepo { 
  ...
  @override
  Future<Either<String, AllPostsModel>> getAllPosts({required String token}) async
  {
    try
    {
      final response=await apiConsumer.get(EndPoints.getAllPostsEndPoint(token: token));
  ...
  Future<Either<String, String>> upvatePost({required num postId}) async
  {
    try
    {
      final response=await apiConsumer.post(EndPoints.upvotePostEndPoint(postId: postId));
  ...
  Future<Either<String, String>> downvatePost({required num postId}) async
  {
    try
    {
      final response=await apiConsumer.post(EndPoints.downvotePostEndPoint(postId: postId));
      return Right(response[ApiKeys.message]);
    } on ServerException catch (e)
    {
      return Left(e.errorModel.errorMessage);
  ...
  @override
  Future<Either<String, String>> addComment({required num postId, required String comment}) async 
  {
    try
    {
      final response=await apiConsumer.post(EndPoints.addCommentEndPoint, data:
          {
        ApiKeys.comment_content:comment,
        ApiKeys.post_id:postId
      },
  ...
  {
    try {
      final response=await apiConsumer.delete(EndPoints.deletePostEndPoint(postId: postId));
      return Right(response[ApiKeys.message]);
    } on ServerException catch (e)
    {
     return Left(e.errorModel.errorMessage);
  ...
  @override
  Future<Either<String, String>> addPost({ String? postText, MultipartFile? postImage}) async
  {
    try
    {
  ...
  {
    final response=await apiConsumer.post(EndPoints.searchForPostsEndPoint,data: {
      ApiKeys.query:query
    },
        isFormData: true
    );
  ...
  Future<CommentsModel> getCommnetsOnPost({required num postId}) async {
    ...
    final response=await apiConsumer.get(EndPoints.getCommentsOnPostEndPoint(postId: postId));
  ...
  Future<Either<String, String>> deleteComment({required num commentId}) async {
    try
    {
      final response=await apiConsumer.delete(EndPoints.deleteCommentEndPoint(commentId: commentId));
      return Right(response[ApiKeys.message]);
    }on ServerException catch(e)
    {
      return Left(e.errorModel.errorMessage);
  ...
  @override
  Future<Either<String, String>> updateComment({required String newContent, required num commentId}) async
  {
    try
    {
      final response=await apiConsumer.post(EndPoints.updateCommentEndPoint(commentId: commentId),
          data: {
        ApiKeys.comment_content:newContent
      });
      return Right(response[ApiKeys.message]);
    }on ServerException catch(e)
    {
  ...
  @override
  Future<Either<String, String>> updatePost({required num postId, String? postText, MultipartFile? attachment}) async
  {
    ...
    try
    {
      ...
      final response=await apiConsumer.post(EndPoints.updatePostEndPoint(postId: postId),data: data,isFormData: true);
      return Right(response[ApiKeys.message]);
    } on ServerException catch (e)
    {
      return Left(e.errorModel.errorMessage);
  ...
  Future<Either<String, int>> getCommentsNumber({required num postId}) async {
    try
    {
      final response=await apiConsumer.get(EndPoints.getCommentsNumberEndPoint(postId: postId));
  ...
  Future<Either<String, int>> getDownvotesNumber({required num postId}) async  {
    try
    {
      final response=await apiConsumer.get(EndPoints.getDownvotesNumberEndPoint(postId: postId));
  ...
  Future<Either<String, int>> getUpvotesNumber({required num postId}) async
  {
    try
    {
      final response=await apiConsumer.get(EndPoints.getUpvotesNumberEndPoint(postId: postId));
  ...
  Future<Either<String, OnePostModel>> getOnePost({required num postId}) async
  {
    try
    {
      final response=await apiConsumer.get(EndPoints.getOnePostEndPoint(postId: postId));
